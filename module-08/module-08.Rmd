...
mean(m) # this is the mean of the sampling distribution = average sample mean; this should be really close to the population mean
```

#### The Standard Error 

The *standard deviation of the sampling distribution*, i.e., of all possible means of samples of size *n* from a population, is referred to as the *standard error*. If the population standard deviation (sigma) is known, then the standard error can be calculated as **population standard deviation / sqrt(sample size)**

``` {r}
sigma/sqrt(n)
```

If the true population standard deviation isn't known, however, the standard error can be estimated from the standard deviation of a given sample (or, presumably, as the average standard deviation across samples). Recall that the *standard error of the mean* for a sample is:
...
A general way to define a confidence interval is as:
	...
	the *statistic* being considered (e.g., the mean) ± some *critical value* $\times$ the *standard error* of the statistic,

where the statistic is the sample statistic under scrutiny, the critical value is a value derived from the standardized version of the sampling distribution (normal, Student's t, Chi Square, etc.) that corresponds to the 100 $\times$ (1 - $\alpha$ level), e.g., 100 $\times$ (1-0.05) for the 95% CI, and the *standard error* is the standard deviation of the sampling distribution (which, as noted above, is often estimated from the sample itself).
