...
You find a cumulative probability for a continuous random variable by calculating the area under the density function of interest from −∞ to $x$. This is what is returned from `pbeta()` [the other related functions, `rbeta()`, `dbeta`, and `qbeta()`] are also useful. The first, draws random observations from that distribution, the 

Finally, the **survival function** of a random variable $X$ is defined as
$S(x) = P(X > x)$ = 1 - $F(x)$




For continuous random variables, the PDF is equivalent to the derivative of the CDF.
...
## Mean of a Discret Random Variable

We can calculate the mean (or expected) value of for a random varible with a given probability mass function as follows:

$\mu X$ = Expectation of $X$ = $\sum_{i=1}^k x_i\times Pr(X=x_i)$
