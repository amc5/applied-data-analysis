...
A quantile-quantile or "Q-Q" plot can be used to look at whether your data seem to follow a normal distribution. A Q–Q plot is a graphical method for comparing two probability distributions. Thus, to examine a set of data for normality graphically, you can plot the quantiles for the actual data (y values) versus the theoretical quantiles (x values) pulled from a normal distribution. If the two distributions being compared are similar, the points in the plot will approximately lie on the line y = x

In this case, they **should** since you have simulated from such a distribution. Simply apply the two ***R*** functions `qqnorm()` and `qqline()` using the vector of data you want to examine as an argument.
...
``` {r} 
theor_quant <- qnorm(ppoints(length(v))) # calculates quantiles from as many probability points drawn from a normal distribution
obs_quant <- quantile(v,ppoints(v)) # calculates the observed quantiles from the vector v
plot(theor_quant,obs_quant)
```
