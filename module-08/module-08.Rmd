...
$f(x)$ = $\binom{n}{k}$ $p^k(1-p)^{n-k}$ where $x$ = {0, 1, 2, ... , n}
...
plot(x,punif(q=x,min=a,max=b),type="l",xlab="x",ylab="Pr(X ≤ x)",main="Cumulative Probability") # the CDF increases linearly over the given interval
```

#### CHALLENGE 5:
Simulate a sample of 10000 random numbers from a uniform distribution in the interval between $a$ = 6 and $b$ = 8. Calculate the mean and variance of this simulated sample and compare it to the expectation for these parameters, i.e., $\mu_X$ = $\frac{b-a}{2}$ and $\sigma_X^2$ = $\frac{(b-a)^2}{12}$.
...
manipulate(
	plot(seq(from=(mu - 6*sigma), to=(mu + 6*sigma), length.out = 1000),
			 ...
			 type="l",xlim=c(-20,20), xlab="x",ylab="f(x)",
			 main="Probability Density Function") +
		abline(h=0) + abline(v=c(qnorm((1-CI)/2,mean=mu,sd=sigma),qnorm(1-(1-CI)/2,mean=mu,sd=sigma))),
	mu=slider(-10, 10, initial=0, step=0.25),
	sigma=slider(0.25, 10, initial=1, step=0.25),
	CI=slider(0.50,0.99,initial=0.90, step=0.01)
)
```
