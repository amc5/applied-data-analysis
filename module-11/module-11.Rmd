...
typeII <- function(mu0,muA,sigma,n,type="two-tailed",alpha=0.05,k=1000){
	...
	if (type == "greater") {
	 polygon(cbind(
	   c(muA-4*sigma/sqrt(n), seq(from=muA-4*sigma/sqrt(n), to=mu0+qt(1-alpha)*sigma/sqrt(n), length.out=100), mu0+qt(1-alpha)*sigma/sqrt(n))),
	   c(0,dt(seq(from=muA-4*sigma/sqrt(n), to=mu0+qt(1-alpha)*sigma/sqrt(n), length.out=100),
	             mean=muA,sd=sigma/sqrt(n)),0), border="red",col="red")
	  abline(v=mu0+qt(1-alpha)*sigma/sqrt(n), col="black", lty=3, lwd=2)
	  q<- pnorm(mu0+qt(1-alpha,df=n-1)*sigma/sqrt(n),mean=muA,sd=sigma/sqrt(n))
	}
	if (type == "less") {
	 polygon(cbind(
	   c(mu0-qnorm(1-alpha)*sigma/sqrt(n), seq(from=mu0-qnorm(1-alpha)*sigma/sqrt(n), to=muA+4*sigma/sqrt(n), length.out=100), muA+4*sigma/sqrt(n))),
	               c(0,dnorm(seq(mu0-qnorm(1-alpha)*sigma/sqrt(n), to=muA+4*sigma/sqrt(n), length.out=100),
	                         mean=muA,sd=sigma/sqrt(n)),0), border="red",col="red")
		...
		if (type == "two-tailed") {
	 # less
	 if (muA<=mu0){polygon(cbind(
	   c(mu0-qnorm(1-alpha/2)*sigma/sqrt(n), seq(from=mu0-qnorm(1-alpha/2)*sigma/sqrt(n), to=muA+4*sigma/sqrt(n), length.out=100), muA+4*sigma/sqrt(n))),
	               c(0,dnorm(seq(mu0-qnorm(1-alpha/2)*sigma/sqrt(n), to=muA+4*sigma/sqrt(n), length.out=100),
	                         mean=muA,sd=sigma/sqrt(n)),0), border="red",col="red")
		  }
		  # greater	 
		  if (muA>=mu0){polygon(cbind(
	   c(muA-4*sigma/sqrt(n), seq(from=muA-4*sigma/sqrt(n), to=mu0+qnorm(1-alpha/2)*sigma/sqrt(n), length.out=100), mu0+qnorm(1-alpha/2)*sigma/sqrt(n))),
	   c(0,dnorm(seq(from=muA-4*sigma/sqrt(n), to=mu0+qnorm(1-alpha/2)*sigma/sqrt(n), length.out=100),
	             mean=muA,sd=sigma/sqrt(n)),0), border="red",col="red")
		  }
	  abline(v=c(mu0-qnorm(1-alpha/2)*sigma/sqrt(n),mu0+qnorm(1-alpha/2)*sigma/sqrt(n)), col="black", lty=3, lwd=2)
	  q<- pnorm(mu0+qnorm(1-alpha/2)*sigma/sqrt(n),mean=muA,sd=sigma/sqrt(n))
	}
...
e<-typeII(mu0=4,muA=5,sigma=3,n=18,alpha=0.05,type="two-tailed")
e
```
