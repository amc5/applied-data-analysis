...
First, let's set up a skeleton function we will call `typeI()` to evaluate the Type I error rate. It should take, as arguments, the parameters of the normal distribution for the null hypothesis we want to simulate from ($\mu_0$ and $\sigma$), our sample size, our alpha level, and the number of simulated datasets we want to generate. Type in the code below (and note that we set default values for $\alpha$ and the number of simulations).
...
``` {r}
e <- typeI(mu0=3, sigma=2, alpha = 0.05, n=1000)
e
```

#### CHALLENGE:
How does the Type I error rate change with $n$ and with $\sigma$?
...
``` {r}
e <- typeII(mu0=0, muA=4, sigmaA=4, n=20)
e
```
...
for (i in 1:length(n)) {
	for (j in 1:length(s)){
	p[i,j] <- typeII(mu0=2,muA=4,sigmaA=j,alpha=0.05,n=i)
	}
}
p <- cbind(p,n)
manipulate(
