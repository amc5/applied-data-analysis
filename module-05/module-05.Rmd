...
This data set consists of basic statistics (area, current population size, birth rate, death rate, life expectancy, and form of government) for 249 countries taken from [**https://www.worlddata.info**] (https://www.worlddata.info) that I have combined with data from the International Union for the Conservation of Nature (IUCN)'s [Red List Summary Statistics] (http://www.iucnredlist.org/about/summary-statistics) about the number of threatened animal species by country.
...
# you can change the sep argument as needed to use other delimiters
head(d)
# the head function lists the first 6 lines of data
# with bracket notation you can modify this (e.g., head(d)[1:10])
tail(d)
# the tail function works similarly, to show the end of your data
class(d)
# shows you that tables are typically loaded in as data frames
```
...
The {XLConnect} package can also write data frames back out to Excel worksheets. If the file does not exist, it is created. If it does exist, data is cleared and overwritten. The second process is MUCH slower. I have included (but commented out) the `file.remove()` command here, in case it is useful.
...
To load data from a `.csv` located file in your *personal* **Dropbox** account you can use the {rdrop2} package.
...
``` {r}
# the same updated link to the raw data you used above is what you need here, too
require(repmis)
